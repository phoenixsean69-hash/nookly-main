...
const LocationPickerMap = ({
  ...
  const initial =
    initialCoords &&
    isValidCoordinate(initialCoords.latitude, initialCoords.longitude)
      ? initialCoords
      : null;
...
(function(){
  ...
  const toggleMapType = () => {
    ...
    webViewRef.current?.injectJavaScript(
      `window.setMapType && window.setMapType('${nextType}');true;`,
    );
  };
  ...
  const reverseGeocode = async (latitude: number, longitude: number) => {
    ...
    try {
      ...
      if (!value) {
        ...
        setAddress(blank);
        setAddressPreview(
          "Address not found. The coordinates can still be used.",
        );
        return blank;
      }
      const result: AddressFields = {
        ...
        neighbourhood:
          value.suburb || value.neighbourhood || value.district || "",
        cityTown:
          value.city || value.town || value.village || value.state || "",
        formattedAddress: "",
      };
      ...
      setAddress(result);
      setAddressPreview(
        result.formattedAddress || data.display_name || "Location selected",
      );
      return result;
    } catch {
      const blank = emptyAddress();
      setAddress(blank);
      setAddressPreview(
        "Address unavailable. The coordinates can still be used.",
      );
      return blank;
    } finally {
  ...
  const handleMapMessage = (event: WebViewMessageEvent) => {
    try {
      ...
      if (message.type === "ready") {
        ...
        if (mapType === "hybrid") {
          webViewRef.current?.injectJavaScript(
            `window.setMapType && window.setMapType('hybrid');true;`,
          );
        }
    ...
    try {
      ...
      if (status !== "granted") {
        Alert.alert(
          ...
          "Allow location access to place the pin at your current position.",
        );
        return;
      }
      ...
      Alert.alert(
        ...
        "Nookly could not get your current location. You can still tap the map.",
      );
    } finally {
      setLocating(false);
    }
  ...
  return (
    ...
    >
      <SafeAreaView style={{ flex: 1, backgroundColor: theme.background }}>
        ...
        {!!searchError && (
          <Text className="px-4 py-2 text-sm text-red-500">{searchError}</Text>
        )}
        ...
        <View className="flex-1">
          <WebView
            ...
            onHttpError={(event) => {
              if (event.nativeEvent.statusCode >= 400) setMapFailed(true);
            }}
            javaScriptEnabled
            domStorageEnabled
          ...
          {mapReady && !mapFailed && (
            <TouchableOpacity
              ...
              accessibilityLabel={
                mapType === "street"
                  ? "Switch to hybrid satellite view"
            ...
            >
              <Ionicons
                name={mapType === "street" ? "globe-outline" : "map-outline"}
                size={16}
                color={theme.primary[300]}
              />
              <Text
                className="ml-2 font-rubik-medium text-xs"
                style={{ color: theme.text }}
              >
          ...
          {mapFailed && (
            ...
            >
              ...
              <Text
                ...
                style={{ color: theme.muted }}
              >
                Check your internet connection, then retry. This screen will not
                close or crash.
              </Text>
              <TouchableOpacity
                onPress={retryMap}
        ...
        >
          ...
          <View className="mt-1 min-h-12">
            {pin ? (
              ...
              <Text
                ...
                style={{ color: theme.muted }}
              >
                Tap anywhere to choose the exact property location.
              </Text>
            )}
          </View>
          <View className="mt-3 flex-row gap-3">
            ...
            <TouchableOpacity
              ...
              className="flex-[1.4] rounded-full py-3"
              style={{
                backgroundColor: pin ? theme.primary[300] : `${theme.muted}40`,
              }}
            >
