...
const backupSuffix = ".secure-lease-flow-v2.bak";
...
function patchNotificationScreen(original) {
  let content = original;
  ...
  if (
    ...
    const requestBlock = `      } else if (notification.type === "request") {
    ...
    const leaseBlock = `      } else if (notification.type === "request") {
        const propertyId = notification.data?.propertyId;
        if (propertyId) {
          router.push(\`/properties/\${propertyId}\`);
        } else {
    ...
    content = replaceRequired(
      ...
      requestBlock,
      leaseBlock,
      "tenant lease notification navigation",
    );
  }
...
`;
    ...
    content = replaceRequired(
      ...
      marker,
      `${helper}${marker}`,
      "the lease notification card insertion point",
    );
  }
  ...
  if (
    ...
    content = replaceRequired(
      ...
      `          <Text className="text-sm mt-1" style={{ color: theme.muted }}>
          ...
          {renderLeaseDetails(item)}
          <Text className="text-xs mt-1" style={{ color: theme.muted }}>`,
      "the lease notification card placement",
    );
  }
...
console.log("");
console.log(
  "Secure Nookly lease flow v2 applied.",
);
console.log("");
console.log("Updated:");
