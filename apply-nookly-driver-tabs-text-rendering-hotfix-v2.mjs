...
async function assertExpectedProject() {
  ...
  const currentLayout = await readFile(
    path.join(root, "app/(root)/(driver)/_layout.tsx"),
    "utf8",
  );

  if (!currentLayout.includes("DriverTabsLayout")) {
    throw new Error("The current driver tab layout was not recognized.");
  }
}
...
async function main() {
  console.log(
    "\nInstalling Nookly driver tabs + Android text rendering hotfix v2...\n",
  );
  ...
  console.log("\n✓ Driver tabs now match the tenant tab style.");
  console.log("✓ Driver tab labels use full-width Android-safe sizing.");
  console.log("✓ Global Android text-clipping workaround installed.");
