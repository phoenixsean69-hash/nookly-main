...
const buildPriceHistory = (property: PropertyData | null) => {
  ...
  const fetchTenantsForProperty = async () => {
    ...
    try {
      ...
      const tenantProfiles = await databases.listDocuments(
        ...
        [Query.equal("currentProperty", property.$id)],
      );
      ...
      const tenantsWithDetails = await Promise.all(
        tenantProfiles.documents.map(async (profile) => {
          try {
            const userDocs = await databases.listDocuments(
              config.databaseId!,
              config.usersCollectionId!,
              [Query.equal("accountId", profile.userId)],
            );
            ...
            if (userDocs.documents.length > 0) {
              ...
              return {
                ...
                name: userDoc.name || "Tenant",
                email: userDoc.email || "",
                phone: userDoc.phone || "",
                avatar: userDoc.avatar || userDoc.customAvatar || null,
  ...
  const handleTenantPress = (tenant: TenantWithProfile) => {
