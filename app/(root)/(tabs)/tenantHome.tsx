      ...
      <View className="px-5">
        ...
        <View className="mb-6">
          <View className="flex-row items-center justify-between mb-4">
            <View>
              <Text className="text-2xl font-rubik-bold" style={{ color: theme.text }}>Featured</Text>
              <Text className="text-sm text-gray-500 font-rubik mt-0.5">Top ranked properties for you</Text>
            </View>
            <TouchableOpacity onPress={onSeeAllFeatured} className="bg-primary-50 px-4 py-2 rounded-full">
              <Text className="text-sm font-rubik-medium" style={{ color: theme.primary[300] }}>See all</Text>
            </TouchableOpacity>
          </View>
          ...
          {loadingFeatured && featuredProperties.length === 0? (
            ...
            <View className="h-48 items-center justify-center rounded-2xl" style={{ backgroundColor: theme.surface }}>
              <Text style={{ color: theme.muted }}>No featured properties yet</Text>
            </View>
          ) : (
            <FlatList
              ...
              renderItem={({ item }) => (
                <View className="mr-4">
                  <FeaturedCard item={item} onPress={() => onCardPress(item.$id)} />
                </View>
              )}
            />
          )}
        ...
        <View className="mb-3">
          <Text className="text-2xl font-rubik-bold" style={{ color: theme.text }}>Recommended</Text>
          <Text className="text-sm text-gray-500 font-rubik">Places you might like</Text>
        </View>
      </View>
    );
  }
...
const Home = () => {
  ...
  const { loadNotifications, fetchAppwriteUnreadCount, totalUnreadCount, cleanupOldNotifications } =
  ...
  const fetchBestProperties = useCallback(async () => {
    const now = Date.now();
    if (now - lastFeaturedFetch.current < STALE_TIME && featuredProperties.length > 0) {
  ...
  useEffect(() => { fetchBestProperties(); }, [fetchBestProperties]);
  ...
  const handleSeeAllFeatured = useCallback(() => setFeaturedModalVisible(true), []);
  ...
  const ListHeaderComponent = useCallback(() => (
    ...
    />
  ), [theme, featuredProperties, loadingFeatured, handleSeeAllFeatured, handleSearchPress, handleCardPress]);

  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.background }}>
      <View className="relative mb-2">
        <Image source={headerImage} className="w-full h-36" style={{ opacity: 0.95 }} />
        <LinearGradient colors={["transparent", "rgba(0,0,0,0.8)"]} style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: "100%" }} />
        <View className="absolute inset-0 flex-row items-center justify-between px-6 pt-2">
          <View className="flex-row items-center">
            {!loadingAvatar? (
              <TouchableOpacity onPress={() => router.push("/profile")}>
                <Image source={user?.avatar? { uri: user.avatar } : icons.person} className="w-14 h-14 rounded-full border-2 border-white" />
              </TouchableOpacity>
            ) : (
            ...
            <View className="ml-3">
              <Text className="text-xs font-rubik text-white/90">{greeting}</Text>
              <Text className="text-lg font-rubik-bold text-white">{user?.name || "Guest"}</Text>
            </View>
          </View>
          <TouchableOpacity onPress={() => router.push("/notifications")} className="bg-white/20 p-2.5 rounded-full">
            <Image source={icons.bell} className="w-5 h-5" style={{ tintColor: "#fff" }} />
            {totalUnreadCount > 0 && (
              <View className="absolute -top-1 -right-1 bg-red-500 rounded-full min-w- h- px-1 items-center justify-center">
                <Text className="text-white text-xs font-bold">{totalUnreadCount > 99? "99+" : totalUnreadCount}</Text>
              </View>
            )}
          </TouchableOpacity>
        </View>
      ...
      <FeaturedModal visible={featuredModalVisible} onClose={() => setFeaturedModalVisible(false)} properties={featuredProperties} onPropertyPress={handleCardPress} />
