...
export default function TenantRequests() {
  ...
  const notificationRequestId = Array.isArray(requestIdParam)
    ? requestIdParam[0]
    : requestIdParam;

  const openedLeaseRequestIdRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    if (
      ...
      openedLeaseRequestIdRef.current === notificationRequestId
    ) {
      return;
    }

    const notificationRequest = requests.find(
      (request) => request.$id === notificationRequestId,
    );

    if (!notificationRequest || !notificationRequest.leaseDocumentId) {
      return;
    }
  ...
  }, [loading, notificationRequestId, requests]);
