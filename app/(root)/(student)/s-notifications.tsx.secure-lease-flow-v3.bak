...
interface NotificationItem {
  ...
  eventId?: string;
  eventDate?: string; // For calendar events
}
...
export default function NotificationsScreen() {
  ...
  // Helper function to check if a calendar event should be marked as read
  const shouldMarkEventAsRead = (notification: NotificationItem): boolean => {
    if (notification.type !== "event" || !notification.eventDate) {
      return true; // Non-event notifications can be marked as read
    }
    ...
    // Only mark as read if the event date has passed (or is today)
    return eventDate <= today;
  };

  // Mark relevant notifications as read when screen is focused
  useFocusEffect(
    useCallback(() => {
      const markExpiredNotificationsAsRead = async () => {
        if (userId && notifications.length > 0) {
          // Find unread notifications that should be marked as read
          const notificationsToMark = notifications.filter(
            (n) => !n.read && shouldMarkEventAsRead(n),
          );

          if (notificationsToMark.length > 0) {
            console.log(
              `🔔 Marking ${notificationsToMark.length} expired notifications as read...`,
            );

            // Separate by source
            const localToMark = notificationsToMark.filter(
              (n) => n.source === "local",
            );
            const appwriteToMark = notificationsToMark.filter(
              (n) => n.source === "appwrite",
            );

            // Mark Appwrite notifications as read
            if (appwriteToMark.length > 0) {
              for (const notif of appwriteToMark) {
            ...
            if (localToMark.length > 0) {
              for (const notif of localToMark) {
                await markLocalAsRead(userId, notif.id);
              }
            }
            ...
            // Update local UI state - only mark the expired ones as read
            setNotifications((prev) =>
              prev.map((n) => {
                const shouldMark = notificationsToMark.some(
                  (m) => m.id === n.id && m.source === n.source,
                );
                return shouldMark ? { ...n, read: true } : n;
              }),
            );

            console.log("✅ Expired notifications marked as read");
          }
        }
      };
    ...
    try {
      ...
      const formattedLocal: NotificationItem[] = localNotifs.map(
        (notif: any) => ({
          ...
          eventDate: notif.eventDate, // Store the event date
  ...
  const handleNotificationPress = async (notification: NotificationItem) => {
    ...
    try {
      ...
      const shouldMark = shouldMarkEventAsRead(notification);

      // Mark as read based on source and if it should be marked
      if (!notification.read && shouldMark) {
      ...
      } else if (!notification.read && !shouldMark) {
        // Show a message that the event hasn't happened yet
        Alert.alert(
          "Event Not Yet",
          `This event is scheduled for ${notification.eventDate}. It will be marked as read after the event date.`,
          [{ text: "OK" }],
        );
        return;
      }
  ...
  const renderNotification = ({ item }: { item: NotificationItem }) => {
    ...
    const canMark = shouldMarkEventAsRead(item);
    ...
    return (
      <TouchableOpacity
        ...
        style={{
          ...
          opacity: isEventInFuture && !item.read ? 0.9 : 1,
        }}
      >
        <View
        ...
        <View className="flex-1">
          ...
          {item.source === "local" && item.eventDate && (
            <View className="mt-1">
              <Text className="text-xs" style={{ color: theme.primary[300] }}>
                📅 {new Date(item.eventDate) > new Date() ? "Upcoming" : "Past"}{" "}
        ...
        {!item.read && (
          <View className="flex-row items-center gap-2">
            {!canMark && item.type === "event" && (
              <View className="bg-orange-500/20 px-2 py-1 rounded-full">
                <Text className="text-orange-500 text-xs">Pending</Text>
              </View>
            )}
