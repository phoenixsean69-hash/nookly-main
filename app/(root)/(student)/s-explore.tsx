...
const Explore = () => {
  ...
  const filteredProperties = useMemo(() => {
    ...
    return properties.filter((property: any) => {
      ...
      if (showHotDeals) {
        const hasDrop =
          property.new_price != null && property.new_price < property.price;
  ...
  const priceDropCount = useMemo(() => {
    return (
      properties?.filter(
        (p: any) => p.new_price != null && p.new_price < p.price,
      ).length || 0
    );
  }, [properties]);
  ...
  const ListHeader = useCallback(
    () => (
      <View className="px-5 pt-5 pb-2">
        <Text
          className="text-xl text-center font-rubik-bold mt-1 mb-4"
            ...
            >
              {BEDROOM_OPTIONS.map((num) => (
                <TouchableOpacity
                  ...
                  onPress={() =>
                    setSelectedBedrooms(
                      selectedBedrooms === num ? undefined : num,
                    )
                  }
                ...
                >
                  <Text
                    style={{
                      color: selectedBedrooms === num ? "#fff" : theme.text,
                    }}
                  >
                    {num}+
                  </Text>
