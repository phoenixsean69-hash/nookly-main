...
export interface AuthState {
  ...
  setOrganization: (organization: Organization | null) => Promise<void>;
...
const useAuthStore = create<AuthState>((set, get) => ({
  ...
  updateUser: async (updates: Partial<User>) => {
    ...
    if (!(await hasInternetConnection())) {
      return {
        success: false,
        error: "Connect to the internet before updating your profile.",
      };
    }
  ...
  updateOrganization: async (updates: Partial<Organization>) => {
    ...
    if (!(await hasInternetConnection())) {
      return {
        success: false,
        error: "Connect to the internet before updating the organization.",
      };
    }
  ...
  hydrate: async () => {
    ...
    try {
      ...
      if (!storedOrganization) {
        storedOrganization = (await getData(
          "organization",
      ...
      if (storedUser) {
        ...
        if (storedOrganization) {
          await get().saveOrganizationToStorage(storedOrganization);
        }
  ...
  clearCache: async () => {
    const accountId = get().user?.accountId;
    ...
    try {
      ...
      await deleteSecureValue(STORAGE_KEYS.AUTH_TOKEN);
  ...
  fetchAuthenticatedUser: async () => {
    ...
    const cachedOrganization = get().organization;
    ...
    const shouldShowLoading = !cachedUser;
  ...
  signIn: async (email: string, password: string) => {
    ...
    try {
      ...
      try {
        ...
        // There may be no existing session.
      }
  ...
  signUp: async (userData: SignUpData) => {
    ...
    let createdUserDocumentId: string | null = null;

    if (!(await hasInternetConnection())) {
      return {
        success: false,
        error: "An internet connection is required to create an account.",
      };
    }
    ...
    try {
      ...
      if (online) {
        try {
          ...
          console.error("Could not delete remote session:", error);
        }
      }
